function makeCalc() {
  ...
  return {
    ...
    onOperator(op) {
      ...
      else if (operator && !s.waiting) { const r = _calc(previous, curr, operator); previous = r; newDisplay = _round(r) }
      setData({ display: newDisplay, previous: _round(previous), operator: op, waiting: true, expression: _round(previous) + ' ' + _opText(op) })
    },
    onEqual() {
